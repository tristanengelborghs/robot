...
All the environments are registered in the `insertion` extension. They start
...
with contextlib.suppress(ImportError):
    import insertion.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~insertion.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from insertion.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `insertion` extension."""
    ...
    if args_cli.show_presets:
        from insertion.utils.preset_cli import enumerate_task_presets
